...
public abstract class CommonConfiguration {
	...
	protected void initFrontEnd() {
		...
		this.dataBlocker = new DataBlocker(10 // blockSizeMs
		);
		this.speechClassifier = new SpeechClassifier(10, // frameLengthMs,
				0.003, // adjustment,
				10, // threshold,
				0 // minSignal
		);
//		this.speechClassifier = new SpeechClassifier();
		...
		this.speechMarker = new SpeechMarker(200, // startSpeechTime,
				500, // endSilenceTime,
				100, // speechLeader,
...
//		this.speechMarker = new SpeechMarker();
		...
		this.premphasizer = new Preemphasizer(0.97 // preemphasisFactor
		);
//		this.premphasizer = new Preemphasizer();
		this.windower = new RaisedCosineWindower(0.46, // double alpha
				25.625f, // windowSizeInMs
				10.0f // windowShiftInMs
		);
//		this.windower = new RaisedCosineWindower();

		this.fft = new DiscreteFourierTransform(-1, // numberFftPoints
				false // invert
		);
		
//		this.fft = new DiscreteFourierTransform();

		this.melFilterBank = new MelFrequencyFilterBank(130.0, // minFreq,
				6800.0, // maxFreq,
				40 // numberFilters
		);
...
//		this.melFilterBank = new MelFrequencyFilterBank();
		...
		this.dct = new DiscreteCosineTransform(40, // numberMelFilters,
				13 // cepstrumSize
		);
...
//		this.dct = new DiscreteCosineTransform();
		...
		this.cmn = new LiveCMN(12.0, // initialMean,
				100, // cmnWindow,
				160 // cmnShiftWindow
		);
		
//		this.cmn = new BatchCMN();
		
		this.featureExtraction = new DeltasFeatureExtractor(3 // window
		);
//		this.featureExtraction = new DeltasFeatureExtractor();
	...
	protected void initRecognizer() {
		...
		this.activeListFactory = new PartitionActiveListFactory(
				absoluteBeamWidth, relativeBeamWidth, logMath);

		this.searchManager = new SimpleBreadthFirstSearchManager(logMath,
				linguist, pruner, scorer, activeListFactory, false, 0.0, 0,
				false);

		this.decoder = new Decoder(searchManager, false, false,
				new ArrayList<ResultListener>(), 100000);

		this.recognizer = new Recognizer(decoder, monitors);

		this.monitors = new ArrayList<Monitor>();
		this.monitors.add(new BestPathAccuracyTracker(recognizer, false, false,
				false, false, false, false));
		this.monitors.add(new MemoryTracker(recognizer, false, false));
		this.monitors.add(new SpeedTracker(recognizer, frontend, true, false,
				false, false));
	}
	...
	public CommonConfiguration() throws MalformedURLException,
			URISyntaxException, ClassNotFoundException {
		init();
	}
